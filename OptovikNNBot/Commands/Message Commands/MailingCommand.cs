...
{
    internal class MailingCommand : ICommand
    {
        public string Name => "/mailing";
        ...
        public async Task Execute(ITelegramBotClient botClient, Update update)
        {
            ...
            if (update.Message?.Chat.Id != null)
            {
                ...
                using (context)
                {
                    ...
                    var text = "Отправьте одно или несколько, которые необходимо включить в рассылку";
                    ...
                    if (currentUser == null)
                    {
                        var newUserState = new UserState { TgUserId = chatId, State = UserTgState.WaitingForMailing.ToString() };
                        userStates.Create(newUserState);
                    }
                    else
                    {
                        currentUser.State = UserTgState.WaitingForMailing.ToString();
