...
{
    internal class DiscOfferNo : ICommand
    {
        public string Name => "discounts_no";
        ...
        public async Task Execute(ITelegramBotClient botClient, Update update)
        {
            ...
            {
                GetSubscriptionCommand getSubscription = new GetSubscriptionCommand();
                var messageId = update.CallbackQuery.Message.MessageId;
                var chatId = update.CallbackQuery.Message.Chat.Id;
                var callbackQueryId = update.CallbackQuery.Id;
                
                await botClient.AnswerCallbackQueryAsync(callbackQueryId);
                var text = $"Вы всегда можете подписаться на рассылку в главном меню на вкладке \"{getSubscription.Name}\"";
